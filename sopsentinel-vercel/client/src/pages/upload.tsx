...
export default function UploadPage() {
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    try {
      // Step 1: Parse PDF in browser (avoids Vercel payload size limit)
      setUploadProgress("Extracting text from PDF (this may take a moment for large files)...");
      const { text, pageCount } = await extractTextFromPDF(file);

      if (!text || text.trim().length < 100) {
        throw new Error("Could not extract meaningful text from this PDF. It may be scanned/image-based.");
      }

      // Step 2: Send extracted text + metadata as JSON (much smaller than PDF binary)
      setUploadProgress("Sending to analysis engine...");
      const res = await apiRequest("POST", "/api/plans/analyze", {
        filename: file.name,
        fileSize: file.size,
        rawText: text,
        pageCount,
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-lg">
                {uploading ? uploadProgress || "Processing..." : "Drop your HMP PDF here"}
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                or click to browse • PDF up to 100MB • Up to 1000 pages
              </p>
            </div>
            {!uploading && (
